...
*Recommendation:* Update all file headers to MPL-2.0 to match LICENSE.txt.
...
just build     # Smart build (Guix → Mix fallback)
----
...
|MIT license header in security.ex |🟡 MEDIUM |Update to MPL-2.0
...
|Mix |Fallback |Direct build when Guix unavailable
|===
...
|1 |License header in `security.ex` says MIT |Change to MPL-2.0
...
1. ✓ Update license headers to MPL-2.0
...
- Cross-platform support via Guix/Burrito
