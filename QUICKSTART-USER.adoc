...
| Elixir 1.17+ / OTP 26+
| `asdf install` (uses `.tool-versions`) or https://elixir-lang.org/install.html
| `elixir --version`
...
* **Rust stable** -- for NIFs: `rustup toolchain install stable`
...
----
cd tma-mark2
asdf install           # installs versions pinned in .tool-versions
mix deps.get
mix assets.setup
